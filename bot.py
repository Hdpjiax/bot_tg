...
def extraer_fecha(texto):
    """Extrae fecha para ordenar vuelos por proximidad real."""
    meses = {
        "ene":1, "feb":2, "mar":3, "abr":4, "may":5, "jun":6, 
        "jul":7, "ago":8, "sep":9, "oct":10, "nov":11, "dic":12
    }
    try:
        # Busca patrones tipo '27 dic' o '27/12' en el texto
        ...
        pass
    return datetime(2099, 12, 31) # Al final si no hay fecha
...
def dashboard_stats():
    ...
    total_recaudado = sum(float(v.get('monto', 0) or 0) for v in vuelos if v['estado'] == 'QR Enviados' or v['estado'] == 'Pago Confirmado')
...
def web_confirmar(v_id):
    ...
    if res.data:
        asyncio.run_coroutine_threadsafe(
            bot_app.bot.send_message(res.data[0]['user_id'], f"✅ Pago Validado\nSu pago para el ID {v_id} ha sido confirmado. En breve enviaremos sus boletos."),
            bot_app.loop
        )
    return redirect(request.referrer)

@app_web.route('/accion/enviar_qr_masivo', methods=['POST'])
def enviar_qr_masivo():
    ...
    if not res.data:
        return jsonify({"status": "error", "message": "Vuelo no encontrado"}), 404
    ...
    for f in files:
        img_bytes = f.read()
        asyncio.run_coroutine_threadsafe(
            bot_app.bot.send_photo(chat_id=uid, photo=img_bytes, caption=f"🎫 Aquí tienes tu boleto/QR (Vuelo #{v_id})"),
            bot_app.loop
        )
    
    supabase.table("cotizaciones").update({"estado": "QR Enviados"}).eq("id", v_id).execute()
    return jsonify({"status": "success", "message": f"QRs enviados al vuelo {v_id}"})
...
if __name__ == "__main__":
    ...
    # Hilo separado para que Flask y el Bot no se bloqueen entre sí
    threading.Thread(target=run_flask, daemon=True).start()
    
    print("🚀 Sistema híbrido (Bot + Dashboard) iniciado correctamente.")
    bot_app.run_polling()
